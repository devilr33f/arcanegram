...
| `LNK1181: cannot open input file '...\\zlib\\Debug\\libzsd.lib'` or `...\\zlibstaticd.lib` after an upstream debug rebase | The prepared zlib artifacts and CMake/Qt import paths disagree across the bump: current `prepare.py` builds `libzsd.lib`, while `cmake/external/qt/package.cmake` may still feed `zlibstaticd.lib` into generated projects. | Run `prepare-deps.bat debug`, then duplicate `Libraries/win64/zlib/Debug/libzsd.lib` to `Libraries/win64/zlib/Debug/zlibstaticd.lib` before retrying `build-telegram.bat debug`. |
...
| `Assertion Failed! "forwarded->originalHiddenSenderInfo != nullptr" history_view_reply.cpp:590` when scrolling past a forwarded message that replies to a hidden user | `feature__hidden-users-reply-preview` overrides `Reply::sender()` to return nullptr for hidden items, but `Reply::senderName(view, data, shorten)` then falls through to the forwarded branch and asserts `originalHiddenSenderInfo` on the resolved (replied-to) message — which can be a normal forward with `originalSender` set instead | Add an early-return at the top of `Reply::senderName(view, data, shorten)`: if `data->resolvedMessage` is from a hidden user, return `tr::ag_hidden_user_name(tr::now)` before walking the sender/forwarded branches. |
| AV `read at 0x20` crashing with `HistoryWidget::messagesReceived` → `History::addOlderSlice` → `Element::computeIsAttachToPrevious+0x4be` on stack when scrolling up in a megagroup | Upstream commit `83ff561794` ("Detach anonymous messages from different authors") added a `rank` lambda in `computeIsAttachToPrevious` that does `item->Get<HistoryMessageSigned>()->isAnonymousRank` without null-checking the component. `isAnonymousRank` is at struct offset 0x20. Other call sites in upstream do null-check. | `misc/rank-null-guard.patch` — wrap the deref: `(msgsigned && msgsigned->isAnonymousRank) ? msgsigned->author : QString()`. Also worth reporting upstream. |
