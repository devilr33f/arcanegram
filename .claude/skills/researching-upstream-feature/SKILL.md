...
| Display name chokepoint | `data/data_peer.cpp` | `PeerData::name()` (~1330, returns `_name`, **non-virtual**), `PeerData::shortName()` (~1339, falls through to `firstName`/`lastName` for users — direct field bypass), `PeerData::topBarNameText()` (~1315, falls back to `user->nameOrPhone` — another bypass). `_name` is composed in `data/data_user.cpp::UserData::setName` from firstName+lastName |
| Direct firstName/lastName bypasses | n/a | (a) typing indicator: `history/view/history_view_send_action.cpp` reads `user->firstName` at ~246–255, ~302, ~348–350 to build "X is typing"; (b) reply preview shorten path: `history/view/history_view_reply.cpp::Reply::senderName` (~605) reads `user->firstName` when `shorten=true`; (c) forwarded sender baked into `HistoryMessageForwarded::originalHiddenSenderInfo->name` at item construction (frozen string) |
| Avatar paint chokepoint | `data/data_peer.cpp` | `PeerData::paintUserpic(QPainter, PeerUserpicView, ctx)` (~452) is the single funnel — `paintUserpicLeft()` and 2-arg overload delegate here; monoforum forwards to broadcast peer. Hook to short-circuit to `EmptyUserpic`. Empty userpic background = `Ui::EmptyUserpic::UserpicColor(colorIndex())` (~empty_userpic.cpp:278–287, 8 gradient pairs); initials picked by `EmptyUserpic::fillString()` (~600) from `peer->name()` — so overriding `name()` automatically updates initials |
| Direct cloud-photo bypass (avatar) | n/a | (a) `calls/calls_userpic.cpp` (~133, ~162) reads `peer->userpicCloudImage(view)` for fullscreen call; (b) `boxes/peers/prepare_short_info_box.cpp` (~326) reads `userpicPhotoId()` for profile photo carousel; (c) `info/userpic/` emoji userpic builder (own-photo edit, low risk) |
| Style decoration chokepoints | `data/data_peer.{h,cpp}` | `colorIndex()` (uint8) feeds `ChatStyle::FromNameFg()` (`ui/chat/chat_style.h:642`) consumed at `history_view_message.cpp:1824` (sender), `:4257` (forward header), `history_view_reply.cpp:828` (reply quote bar). `backgroundEmojiId()` / `profileBackgroundEmojiId()` → reply pattern + profile cover. `emojiStatusId()` → premium star. Verified/Scam/Fake/Premium badges centralize at `info/profile/info_profile_values.cpp::BadgeContentForPeer` (~276) and `BadgeValue` (~708) — overriding here strips badges from profile, dialog row, chat header in one place |
| Identity exposure surfaces (profile) | `info/profile/info_profile_values.cpp` | `PhoneValue` (~126), `UsernameValue` (~159, also `peer->username()` → `t.me/<u>` link via `UsernamesLinkCallback` at `info_profile_actions.cpp:183`), `PlainAboutValue` (~45), `BirthdayValueText` (`info_profile_actions.cpp:901`), `CommonGroupsCountValue` (~600), peer ID display at `info_profile_actions.cpp:216–228` (existing `ShowPeerIdBelowAbout` toggle); last-seen text at `data/data_peer_values.cpp::OnlineTextFull` (~481–501) |
| Identity exposure surfaces (lists) | various | dialogs search row username at `dialogs/dialogs_inner_widget.cpp` (~1613, ~1758); `@username` in autocomplete at `chat_helpers/field_autocomplete.cpp::PrimaryUsername` (~60–63); username embedded in own-user notification text at `window/notifications_manager.cpp` (~1253–1255). Inline `@username` mentions inside message body text are user-typed content — cannot anonymize without corrupting message text |
| Inline-bot ("via @bot") detection | `history/history_item_components.h:70` | `HistoryMessageVia` runtime component, fields `UserData *bot`, `QString text`, `ClickHandlerPtr link`. Accessor `HistoryItem::viaBot()` at `history/history_item.cpp:1913` returns `UserData*` or `nullptr`. Component is attached only when MTProto `via_bot_id` was set on the message (parsed at `history_item.cpp:4738`, allocated at `:4354`) — true inline-bot case, NOT regular bot DMs. `bot->id` is a `PeerId`; convert with `peerToUser()` for storage. Render at `history_view_message.cpp:1902` (inline) and `:2226` (`paintViaBotIdInfo`, standalone). For regex/text matching against item body+caption use `HistoryItem::originalText().text` (single field) |
| Per-message context-menu hook | `history/view/history_view_context_menu.cpp:1058` | `AddMessageActions(menu, request, list)` — single chokepoint where the fork can append message-scoped items (e.g. "Hide messages via @bot" guarded by `request.item->viaBot()`). `request` carries `HistoryItem*` and the `ListWidget*` |
| Right-click message-bubble menu (parallel hierarchies) | `history/history_inner_widget.cpp:2447` `HistoryInner::showContextMenu` vs `history/view/history_view_context_menu.cpp:1519` `FillContextMenu` (called from `history_view_list_widget.cpp:3058`) | **Two completely separate menu builders.** Regular 1:1 / group / channel chats use `HistoryInner::showContextMenu` (~600 lines, builds `_menu` ad-hoc). Threads, scheduled, saved, search results use `ListWidget::FillContextMenu` → `FillContextMenuItems`. A hook in only one of them silently misses half the surfaces. To insert at the **top** of the menu (just below reactions strip), add right after `_menu = base::make_unique_q<...>` (or after the `linkUserpicPeerId` early-return for HistoryInner) and before `AddReplyToMessageAction` for ListWidget. Reactions are attached as a separate header widget by `AttachSelectorToMenu` near the popup call, not as a menu action — menu order does NOT control whether items appear above/below the reactions strip |
| Date dividers + sticky scroll-date | `history/history_inner_widget.cpp:981` `enumerateDates` (paint loop at `:1483`) | Each `Element` may carry a `HistoryView::DateBadge` runtime component (assigned at attach-time by `Element::recountDisplayDateInBlocks`, `history_view_element.cpp:2170`); the same enumerator ALSO synthesizes a sticky scroll-date for whatever non-empty item is at `_visibleAreaTop`. **Hidden-state changes do NOT re-run the recount** — only `previousInBlocksChanged` does. Result: hidden views with leftover DateBadges (or hidden views sitting at the viewport top) cause duplicate "May 7" labels. Fix is `if (view->isHidden()) return true;` at the top of the dateCallback to skip both paths |
| `style::icon` forward-decl | `ui/style/style_core_types.h:34` | `using icon = internal::Icon;` — can't forward-declare in headers without redeclaring the alias. Cleanest workaround: include `ui/style/style_core_types.h` directly in the fork header (lightweight, no Qt widget deps) when the API exposes `const style::icon *` |
| `Ui::Menu::CreateAddActionCallback(menu)` | `lib_ui/ui/widgets/menu/menu_add_action_callback_factory.h:20` | Factory that returns a `MenuCallback` (3-arg: text, callback, icon) — use this to bridge any `_addAction(text, cb, icon)` slot in fork code instead of writing the lambda by hand. Existing fork callbacks (`AddActionCallback` in `ag_hidden_users.h`) match this 3-arg shape |
