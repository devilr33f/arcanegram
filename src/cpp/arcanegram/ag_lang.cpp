...
"ag_streamer_mode_about" = "Запрещает другим приложениям захватывать окно Telegram через скриншоты, запись экрана или через стриминг. Использует Windows API отображаемого приоритета окна (WDA_EXCLUDEFROMCAPTURE). Не работает на Windows младше 10 2004.";
