...
function parsePatchName(patchName: string) {
  const parts = patchName.split('__').map(s => s.trim()).filter(Boolean)
  if (parts.length !== 2) throw new Error(`patch name must use "group__name": ${patchName}`)
  const [group, name] = parts
  return { group, name, seriesEntry: `${group}/${name}.patch` }
}
...
async function listFilesRecursive(dir: string): Promise<string[]> {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) { files.push(...await listFilesRecursive(fullPath)); continue }
    files.push(fullPath)
  }
  return files
}
...
async function clearExportedPatchFiles() {
  for (const file of await listFilesRecursive(patchesDir)) {
    if (file.endsWith('.patch')) await fs.unlink(file)
  }
}
...
async function exportPatchFile(repoDir: string, patchName: string) {
  ...
  if (files.length === 0) warn(`patch ${patchName} is empty`)
