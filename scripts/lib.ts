...
export function step(message: string) { console.log(`${chalk.blue('==>')} ${message}`) }
export function success(message: string) { console.log(`${chalk.green('ok')} ${message}`) }
export function warn(message: string) { console.log(`${chalk.yellow('warn')} ${message}`) }

export function cd(cwd: string) { return $({ cwd }) }
...
export async function ensureUpstreamRemote(repoDir: string) {
  ...
  const remotes = (await git`git remote`).stdout.split(/\r?\n/).map(s => s.trim()).filter(Boolean)
...
async function ensureSymlinkOrCopy(targetPath: string, srcPath: string, type: 'dir' | 'file') {
  ...
  try {
    await fs.symlink(relative(dirname(targetPath), srcPath), targetPath, 'file')
  }
  catch {
...
async function linkForkEntry(repoDir: string, entry: ResolvedLink) {
  ...
  const created = await ensureSymlinkOrCopy(targetPath, entry.sourcePath, entry.type)
...
export async function ensureStgInstalled() {
  const result = await $({ nothrow: true })`stg --version`
  if (result.exitCode !== 0) {
    throw new Error('stgit (`stg`) not found on PATH. install via Git for Windows / MSYS2 / your package manager.')
  }
}
